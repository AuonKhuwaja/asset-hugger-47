              ...
              <thead>
                <tr className="border-b-2 border-dashed border-border bg-primary text-primary-foreground">
                  {["ID", "Asset", "Type", "Description", "Technician", "Date", "Recurrence", "Cost", "Status", "Actions"].map((h) => (
                    <th key={h} className={`px-4 py-3 text-xs font-bold uppercase tracking-wider text-primary-foreground ${h === "Cost" ? "text-right" : "text-left"} border-r border-dashed border-primary-foreground/30 last:border-r-0`}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
