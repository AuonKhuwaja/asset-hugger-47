import { type LucideIcon, TrendingUp, TrendingDown } from "lucide-react";

interface KpiCardProps {
  title: string;
  value: string;
  subtitle?: string;
  icon: LucideIcon;
  trend?: { value: string; positive: boolean };
  iconGlow?: string;
  gradient?: string;
}

export function KpiCard({ title, value, subtitle, icon: Icon, trend, iconGlow = "icon-glow", gradient }: KpiCardProps) {
  return (
    <div className={`relative overflow-hidden rounded-[1.25rem] p-5 animate-scale-in transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl ${gradient ? '' : 'vision-card vision-card-hover'}`}
      style={gradient ? {
        background: gradient,
        border: '1.5px solid rgba(255,255,255,0.1)',
      } : undefined}
    >
      {/* Bubble texture overlay */}
      {gradient && (
        <>
          {/* Large soft bubble top-right */}
          <div className="absolute -top-8 -right-8 w-28 h-28 rounded-full bg-white/[0.06] pointer-events-none" />
          {/* Medium bubble bottom-left */}
          <div className="absolute -bottom-5 -left-5 w-20 h-20 rounded-full bg-white/[0.05] pointer-events-none" />
          {/* Small bubble mid */}
          <div className="absolute top-6 right-16 w-8 h-8 rounded-full bg-white/[0.04] pointer-events-none" />
          {/* Tiny accent bubble */}
          <div className="absolute bottom-8 right-8 w-5 h-5 rounded-full bg-white/[0.07] pointer-events-none" />
          {/* Another tiny bubble */}
          <div className="absolute top-12 left-10 w-4 h-4 rounded-full bg-white/[0.03] pointer-events-none" />
          {/* Shine sweep */}
          <div className="absolute inset-0 bg-gradient-to-br from-white/[0.08] via-transparent to-transparent pointer-events-none" />
        </>
      )}
      <div className="relative flex items-start justify-between">
        <div className="space-y-1.5">
          <p className="text-xs font-semibold uppercase tracking-wider text-white/60">{title}</p>
          <p className="text-[28px] font-bold tabular-data text-white leading-none">{value}</p>
          {subtitle && <p className="text-xs text-white/50 mt-1">{subtitle}</p>}
          {trend && (
            <div className={`flex items-center gap-1 text-xs font-semibold mt-1 ${trend.positive ? "text-emerald-300" : "text-red-300"}`}>
              {trend.positive ? <TrendingUp className="w-3.5 h-3.5" /> : <TrendingDown className="w-3.5 h-3.5" />}
              <span>{trend.value}</span>
            </div>
          )}
        </div>
        <div className={`w-12 h-12 flex items-center justify-center rounded-xl shadow-lg ${gradient ? 'bg-white/15 backdrop-blur-sm' : iconGlow}`}>
          <Icon className="w-5 h-5 text-white" />
        </div>
      </div>
    </div>
  );
}
